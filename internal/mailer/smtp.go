...
func (m *SMTPMailer) send(toEmail, toName, subject, htmlBody string) error {
	msg := mail.NewMsg()
...
func (m *SMTPMailer) SendDecisionEmail(toEmail, toName string, decision Decision) error {
	...
	return m.send(toEmail, toName, fmt.Sprintf(subjectFormat, m.hackathonName), htmlBody)
}
...
func (m *SMTPMailer) SendDecisionsReleasedEmail(toEmail, toName string) error {
	...
	return m.send(toEmail, toName, fmt.Sprintf("%s decisions are out", m.hackathonName), htmlBody)
}
...
func (m *SMTPMailer) SendQREmail(toEmail, toName, userID string) error {
	...
	if err := tmpl.Execute(&htmlBody, qrEmailData{Name: toName, HackathonName: m.hackathonName, From: m.fromName}); err != nil {
...
func (m *SMTPMailer) SendWalkInQueuedEmail(toEmail string, position int) error {
	...
	var htmlBody bytes.Buffer
	if err := tmpl.Execute(&htmlBody, walkInQueuedData{Email: toEmail, Position: position, HackathonName: m.hackathonName, From: m.fromName}); err != nil {
...
func (m *SMTPMailer) SendWalkInAcceptedEmail(toEmail, userID string) error {
	...
	var htmlBody bytes.Buffer
	if err := tmpl.Execute(&htmlBody, walkInAcceptedData{Email: toEmail, HackathonName: m.hackathonName, From: m.fromName}); err != nil {
	...
	msg.Subject(fmt.Sprintf("You're in for %s", m.hackathonName))
	msg.SetBodyString(mail.TypeTextHTML, htmlBody.String())
	if err := msg.AttachReader("hackutd-qrcode.png", bytes.NewReader(qrPNG), mail.WithFileContentType("image/png")); err != nil {
		return fmt.Errorf("attaching QR code: %w", err)
	}
